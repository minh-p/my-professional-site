...
#### The Math for Region Crossing - Is player crossing? To what grid? {#the-math-for-region-crossing-is-player-crossing-to-what-grid}
...
-  Region Crossing Math [Implemented](https://github.com/minh-p/roblox-grid-based-server-sharding/blob/main/src/server/Region/RegionManager.luau)
...
#### The Math for Region Crossing - Player's Relative Position on New Shard {#the-math-for-region-crossing-player-s-relative-position-on-new-shard}
...
-   \\(\Delta{}q = q\_f - q\_s,\quad \Delta{}r = r\_f - r\_s\\)

Hence, our final derived formula is:

-   \\(x\_{new} = s\sqrt{3} (\Delta{}q + \frac{\Delta{}r}{2})\\)
...
-   Let \\(\theta\_s\\) be the source yaw in radians. (Yaw is rotation in the x-z plane).
-   \\(d \in{} (0, 1, 2, 3, 4, 5)\\)
-   \\(\Delta{} = \frac{\pi}{3}\\)

The formula is:

-   \\(\theta\_d = \theta\_s + d\Delta\\)

Implementation, CFrame formula is:

-   \\(C\_d = T(p) R\_y(\theta\_d)\\)

    Where \\(T(p)\\) is translational position and \\(R\_y(\theta\_d)\\) is rotation about Y (or the yaw).
